...
import Session from './hybrid_session'
import SyncData from './hybrid_sync_data'
...
export default class HybridUser extends BaseModel {
  ...
  @column()
  public preferences: object // User preferences for hybrid platform

  @column()
  public metadata: object // Additional metadata for hybrid features
  ...
  public syncData: HasMany<typeof SyncData>
  ...
  public progress: HasMany<typeof UserProgress>
   ...
   */
  public async getPendingSyncData() {
    return await this.related('syncData')
      .query()
      .where('status', 'pending')
      .where('direction', 'offline_to_online')
      .exec()
  }
  ...
  public async getEnrolledCourses() {
    return await this.related('courses').query()
      .where('isActive', true)
      .exec()
  }
   ...
   */
  public async getCourseProgress(courseId: number) {
    return await this.related('progress')
      .query()
   ...
   */
  public async isEnrolledInCourse(courseId: number): Promise<boolean> {
    const enrollment = await this.related('courses')
      .query()
      .where('id', courseId)
      .first()
   ...
   */
  public async enrollInCourse(courseId: number) {
    const course = await Course.find(courseId)
    if (!course) {
      throw new Error('Course not found')
    }

    await this.related('courses').attach([courseId])
  }

  /**
   * Unenroll user from course
   */
  public async unenrollFromCourse(courseId: number) {
    await this.related('courses').detach([courseId])
  }
